        ...
        <div class="row">
            <div class="col-sm-4 col-12 filter-box">
                <div class="row search-box">
                    <input type="text" class="search-input" v-model="foodObj.name" placeholder="Пошук.." />
                </div>

                <div class="row filter-drop-down">
                    <p @click="displayFilterDrop">Фільтрувати<span v-if="showDropDown">x</span><span v-else>v</span></p>
                </div>
                ...
                <div class="row filter-section">
                    <ul class="filter-option">
                        <li>
                            <input type="button" name="cbStatus" id="bsStatus" value="Популярні" hidden
                                @click="filterStatusBtn($event)" />
                            <label for="bsStatus" class="d-flex justify-content-between">Популярні
                                <button class="unselect-btn" @click="unselectStatusBtn($event)"
                                    value="Популярні">X</button></label>
                        </li>


                        <li>
                            <input type="button" name="cbStatus" id="ooStatus" value="Лише онлайн" hidden
                                @click="filterStatusBtn($event)" />
                            <label for="ooStatus" class="d-flex justify-content-between">Лише онлайн <button
                                    class="unselect-btn" @click="unselectStatusBtn($event)"
                                    value="Лише онлайн">X</button></label>
                        </li>


                        <li>
                            <input type="button" name="cbStatus" id="soStatus" value="Знижка" hidden
                                @click="filterStatusBtn($event)" />
                            <label for="soStatus" class="d-flex justify-content-between">Знижка <button
                                    class="unselect-btn" @click="unselectStatusBtn($event)"
                                    value="Знижка">X</button></label>
                        </li>


                        <li>
                            <input type="button" name="cbStatus" id="sdStatus" value="Хіт сезону" hidden
                                @click="filterStatusBtn($event)" />
                            <label for="sdStatus" class="d-flex justify-content-between">Хіт сезону <button
                                    class="unselect-btn" @click="unselectStatusBtn($event)"
                                    value="Хіт сезону">X</button></label>
                        </li>


                        <li>
                            <input type="button" name="cbStatus" id="ndStatus" value="Нові страви" hidden
                                @click="filterStatusBtn($event)" />
                            <label for="ndStatus" class="d-flex justify-content-between">Нові страви <button
                                    class="unselect-btn" @click="unselectStatusBtn($event)"
                                    value="Нові страви">X</button></label>
                        </li>
                ...
                <div class="row filter-section">
                    <ul class="filter-option">
                        <li>
                            <input type="button" name="rPrice" id="rtfPrice" value="20,75" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="rtfPrice" class="d-flex justify-content-between">20 - 75 ₴<button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rPrice" id="rftPrice" value="75,100" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="rftPrice" class="d-flex justify-content-between">75 - 100 ₴<button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rPrice" id="rttPrice" value="100,200" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="rttPrice" class="d-flex justify-content-between">100 - 200 ₴<button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rPrice" id="mtPrice" value="200" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="mtPrice" class="d-flex justify-content-between">{{ ">" }} 200 ₴<button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>
                ...
                <div class="row filter-section">
                    <ul class="filter-option">
                        <li>
                            ...
                            <label for="mType" class="d-flex justify-content-between">М'ясні<button class="unselect-btn"
                        ...
                        <li>
                            <input type="button" name="rType" id="vType" value="vegan" hidden
                                @click="filterTypeBtn($event)" />
                            <label for="vType" class="d-flex justify-content-between">Веганські<button class="unselect-btn"
                                    @click="unselectTypeBtn($event)">X</button></label>
                        </li>
            ...
            <div class="col-sm-8">
                <div class="row">
                    <div class="menu-tabs">
                        <button class="menu-tab-item" id="allFilterFoodBtn" name="allFilterFoodBtn" 
                            value="all" @click="filterFoodBtn($event)">Все</button>
                        <button class="menu-tab-item" id="main_dishesFilterFoodBtn" name="main_dishesFilterFoodBtn" 
                            value="main_dishes" @click="filterFoodBtn($event)">Основні</button>
                        <button id="soupsFilterFoodBtn" name="soupsFilterFoodBtn" class="menu-tab-item" 
                            value="soups" @click="filterFoodBtn($event)">Супи</button>
                        <button id="saladsFilterFoodBtn" name="saladsFilterFoodBtn" class="menu-tab-item" 
                            value="salads" @click="filterFoodBtn($event)">Салати</button>
                        <button id="bakingFilterFoodBtn" name="bakingFilterFoodBtn" class="menu-tab-item" 
                            value="baking" @click="filterFoodBtn($event)">Випічка</button>
                        <button id="dessertFilterFoodBtn" name="dessertFilterFoodBtn" class="menu-tab-item" 
                            value="dessert" @click="filterFoodBtn($event)">Десерти</button>
</div>
                ...
                <div class="row box-container">
                    <div v-for="(f, index) in currentPageItems" :key="index">
                        <div class="box">
                            ...
                            <div class="content">
                                ...
                                <div class="price">
                                    {{ parseFloat(f.food_price) - parseFloat(f.food_discount) }} грн
                                    <span v-if="parseFloat(f.food_discount) != 0.00"> {{ parseFloat(f.food_price)
                                    }}₴</span>
                                </div>
                                <button class="btn" @click="addItem(index)">Додати</button>
                            </div>
                        </div>
                    </div>
                    <div v-if="!filterFoods.length">
                        <div class="box">
                            <div class="content">
                                <h1 style="color: #057835fa;">Не знайдено!</h1>
                            </div>
                            <div class="image">
...
export default {
    ...
    methods: {
        ...
        checkSale: function (food, statusArray) {
            if (statusArray.includes("Знижка")) {
        ...
        checkBest: function (food, statusArray) {
            if (statusArray.includes("Популярні")) {
                if (food.food_status.includes("Популярні")) {
                    return true;
                }
                else {
                    return false;
        ...
        checkOnl: function (food, statusArray) {
            if (statusArray.includes("Лише онлайн")) {
                if (food.food_status.includes("Лише онлайн")) {
                    return true;
                }
                else {
                    return false;
        ...
        checkSeason: function (food, statusArray) {
            if (statusArray.includes("Хіт сезону")) {
                if (food.food_status.includes("Хіт сезону")) {
                    return true;
                }
                else {
                    return false;
        ...
        checkNew: function (food, statusArray) {
            if (statusArray.includes("Нові страви")) {
                if (food.food_status.includes("Нові страви")) {
                    return true;
                }
                else {
                    return false;
