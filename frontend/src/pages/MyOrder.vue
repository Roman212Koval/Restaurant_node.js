        ...
        <div v-if="filterBills.length > 0" class="my-order-cards">
            <div v-for="b in filterBills.slice().reverse()" class="card" :key="b.bill_id">
                <div class="card-head d-flex flex-wrap flex-sm-nowrap justify-content-between">
                    ...
                    <button @click="sendBillId(b.bill_id)">Деталі замовлення</button>
                </div>

                <div class="d-flex flex-wrap flex-sm-nowrap justify-content-between card-summary">
                    <div class="w-100 text-center py-1 px-2"><span>Стан оплати: </span>{{ b.bill_paid ? 'Оплачено' : 'НЕ сплачено' }}
                    </div>
                    <div class="w-100 text-center py-1 px-2"><span>Статус:</span>{{ " " + avaiableStatus[b.bill_status]
                    }}
                    </div>
                    <div class="w-100 text-center py-1 px-2"><span>Час:</span> {{ b.bill_when }}</div>
                </div>
                <div class="d-flex flex-wrap flex-sm-nowrap justify-content-between card-summary">

                    <div class="w-100 text-center py-1 px-2"><span>Загалом:</span> {{ b.bill_total }}₴</div>
                    <div class="w-100 text-center py-1 px-2"><span>Адреса:</span>{{ " " + b.bill_address }}
                    </div>
                    <div class="w-100 text-center py-1 px-2"><span>Номер:</span>{{ " " + b.bill_phone }}
                    </div>
                </div>
...
export default {
    ...
    data() {
        return {
            avaiableStatus: ["Відмінено", "Підтверджено", "Приготовлено", "Пакування", "Доставка", "Доставлено", "Завершено"],
            allBills: [],
