        ...
        <div class="checkout-form-container">
            <form id="checkoutForm" @submit="handleSubmit" novalidate autocomplete="off">
                <div class="checkout-heading">
                    <h3>Ще кілька кроків, щоб оформити ваше замовлення.<span>Загалом</span></h3>
                    <h3 v-if="user">{{ user.user_name }}, введіть дані для оплати<span>${{ calculateSummaryPrice()[3] }}</span></h3>
                </div>

                <div class="form-group details-group">
                    <h4>Дані доставки</h4>
                    <div class="form-group">
                        <input type="text" name="coPhone" id="coPhone" placeholder="Номер телефону" class="form-control"
                            v-model="checkoutObj.phone" />
                        <p class="error-mess" v-if="errorObj.phoneErr.length > 0">{{ errorObj.phoneErr[0] }}</p>
                    </div>

                    <div class="form-group">
                        <input type="text" name="coAddress" id="coAddress" placeholder="Адреса доставки, Київ"
                ...
                <div class="form-group details-group">
                    ...
                    <div class="form-group">
                        <div class="form-group">
                            <input type="radio" name="payment" value="cash" id="paymentCash"
                                v-model="checkoutObj.paymentMethod" /><span>Готівка</span>
                            <input type="radio" name="payment" value="card" id="paymentCard"
                                v-model="checkoutObj.paymentMethod" /><span>Банківська карта</span>
                        </div>
                    ...
                    <div v-if="checkoutObj.paymentMethod == 'card'">
                        <div class="form-group">
                            <input type="text" name="coCardNum" placeholder="Номер карти" id="coCardNum"
                        ...
                        <div class="form-group">
                            <input v-upcase type="text" name="coCardName" placeholder="Ім'я власника"
                        ...
                        <div class="form-group">
                            <div class="form-control">
                                <span
                                    style="font-size: 1.6rem; position: absolute; margin-left: -5px; margin-top: -11px;">Термін дії:
                                </span>
                                <input
    ...
    methods: {
        ...
        checkForm: function () {
            ...
            if (!this.checkoutObj.phone) {
                ...
                if (!this.checkoutObj.phone.startsWith('380')) {
                    this.errorObj.phoneErr.push('Номери телефону має починатися з 380');
                }

                if (this.checkoutObj.phone.length != 12) {
                    this.errorObj.phoneErr.push('Номер телефону повинен містити рівно 12 цифр');
                }

                if (!/[0-9]{11}/.test(this.checkoutObj.phone)) {
                    this.errorObj.phoneErr.push('Номер телефону може містити лише цифри');
                }
            }

            // Address validate
            if (!this.checkoutObj.address) {
                this.errorObj.addressErr.push('Необхідно ввести адресу');
            }

            // Card validate
            if (!this.checkoutObj.paymentMethod) {
                this.errorObj.payErr.push('Необхідно вибрати спосіб оплати');
            }
            else if (this.checkoutObj.paymentMethod == "card") {
                if (!this.cardObj.number) {
                    this.errorObj.numErr.push('Необхідно ввести номер картки');
                }
                else {
                    if (!this.cardObj.number.startsWith('4')) {
                        this.errorObj.numErr.push('Номери карток Visa мають починатися з 4');
                    }

                    if (this.cardObj.number.length != 16) {
                        this.errorObj.numErr.push('Номер картки Visa повинен містити рівно 16 цифр');
                    }

                    if (!/[0-9]{16}/.test(this.cardObj.number)) {
                        this.errorObj.numErr.push('Номери карток Visa можуть містити лише цифри');
                    }
                }

                if (!this.cardObj.name) {
                    this.errorObj.nameErr.push('ім`я обовязкове');
                }
                else {
                    if (!/^[A-Za-zА-Яа-яІіЇїЄєҐґ]+$/.test(this.cardObj.name.replace(/\s/g, ""))) {
                        this.errorObj.nameErr.push('Лише літери');
}

                }

                if (!this.cardObj.expiryDate) {
                    this.errorObj.exDateErr.push('Необхідно вказати термін дії');
                }


                if (!this.cardObj.cvv) {
                    this.errorObj.cvvErr.push('Необхідно ввести код cvv');
                }
                else {
                    if (this.cardObj.cvv.length != 3) {
                        this.errorObj.cvvErr.push('Cvv-код повинен містити рівно 3 цифри');
                    }

                    if (!/[0-9]{3}/.test(this.cardObj.cvv)) {
                        this.errorObj.cvvErr.push('Код Cvv може містити лише цифри');
                    }
                }
