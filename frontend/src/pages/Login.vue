        ...
        <div class="login-form-container">
            <form id="loginForm" @submit="handleSubmit" novalidate autocomplete="off">
                ...
                <div class="form-group">
                    <input type="email" id="uEmail" name="uEmail" class="form-control" placeholder="Введіть email"
                        v-model="loginObj.email" />
                </div>

                <div class="form-group">
                    <input type="password" id="uPass" name="uPass" class="form-control"
                        placeholder="Введіть пароль" v-model="loginObj.pass" />
                </div>

                <div class="form-group">
                    <input type="submit" value="Підтвердити" class="btn">
                    <p>немає акаунту? <router-link @click="scrollToTop()" to="/register">      створити зараз
                        </router-link>
                    </p>
                </div>
            </form>
    ...
    methods: {
        ...
        async handleSubmit(e) {
            ...
            if (!this.loginObj.email) {
                this.errors.push("Необхідно ввести email");
            }
            else {
                if (!/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$/.test(this.loginObj.email)) {
                    this.errors.push('Некоректний email');
                }
            }


            if (!this.loginObj.pass) {
                this.errors.push('Необхідно ввести пароль');
            }
            ...
            if (!this.errors.length == 0) {
                ...
                if (!this.matchUser) {
                    this.errors.push("Неправильні email або пароль!")
                }
                else {
